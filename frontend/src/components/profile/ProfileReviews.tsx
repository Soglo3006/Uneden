...
function ReviewCard({ r, idx, full }: { r: any; idx: number; full?: boolean }) {
  return (
  ...
  return (
    <>
      ...
      <Card className="p-6 mt-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Latest Ratings</h2>
        ...
        {reviewsLoading ? (
          ...
          <div className="space-y-4">
            {reviews.slice(0, 3).map((r, idx) => (
              <ReviewCard key={r.id || idx} r={r} idx={idx} />
            ))}
            <a href="#all-ratings">
      ...
      {reviews.length > 0 && (
        <Card id="all-ratings" className="p-6 mt-8">
          ...
          <div className="space-y-4">
            {reviews.map((r, idx) => (
              <ReviewCard key={r.id || idx} r={r} idx={idx} full />
            ))}
          </div>
        </Card>
      )}
